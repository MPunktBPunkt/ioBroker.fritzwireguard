...
function sanitizeWgConfig(raw, opts = {}) {
    ...
    if (/^Address\s*=/im.test(cfg) && /\d+\.\d+\.\d+\.\d+\/24/.test(cfg)) {
        ...
        warnings.push('Interface-Address von /24 auf /32 geaendert — verhindert Routing des gesamten Subnetzes ueber WG.');
    }
    if (/^Address\s*=.*:/im.test(cfg)) {
        cfg = cfg.replace(/^(Address\s*=\s*[\d.]+\/\d+)[^\n]*/gim, '$1');
        warnings.push('IPv6-Adresse aus Interface-Block entfernt (nur IPv4-Tunnel).');
    }
    if (/^AllowedIPs\s*=.*:/im.test(cfg)) {
        cfg = cfg.replace(/^AllowedIPs\s*=\s*([^\n]+)$/gim, (line, val) =>
            'AllowedIPs = ' + val.split(',').map(s => s.trim()).filter(s => s && !s.includes(':')).join(', '));
        warnings.push('IPv6-Eintraege aus AllowedIPs entfernt.');
    }
    ...
    if (tunnelHosts.length && /^AllowedIPs\s*=/mi.test(cfg)) {
        ...
        if (allowedMatch && /\/(?:24|16|8)\b/.test(allowedMatch[1])) {
            ...
            warnings.push('AllowedIPs auf Tunnel-Hosts eingeschraenkt (ohne lokale FritzBox): ' + hosts.join(', '));
        }
    }
    return { cfg, warnings };
}
...
function execWg(cmd) {
    ...
    return new Promise((resolve, reject) => {
        exec('sudo -n ' + sudoCmd + ' 2>&1', (err, stdout) => {
        ...
        execWg('/usr/bin/wg show ' + iface).then(stdout => {
            ...
            for (const block of stdout.split(/\n\n/)) {
                ...
                peers.push({
                    ...
                    rx: parseWgBytes((block.match(/transfer: ([\d.]+\s*(?:B|KiB|MiB|GiB)) received/im) || [])[1]),
                    tx: parseWgBytes((block.match(/transfer: [^,\n]+,\s*([\d.]+\s*(?:B|KiB|MiB|GiB)) sent/im) || [])[1])
                });
            }
            const connected = peers.some(p => p.handshake !== 'nie' || p.rx > 0);
            resolve({ ifaceUp: true, connected, peers });
...
class FritzWireguardLogic {
    ...
    async _runConnectionChecks(cfg, tryConnect) {
        ...
        if (wgState.connected) {
            ...
            lines.push('<span style="color:#ff9800">&#9888; WireGuard-Interface aktiv, aber kein Handshake</span> — Endpoint/Config pruefen.');
        } else {
        ...
        if (cfg.fritzUser && cfg.fritzPass) {
            try {
                ...
                if (model) {
                    ...
                    lines.push('<span style="color:#ff9800">&#9888; TR-064: Antwort ohne Modellname</span> — bei gleichem Subnetz (192.168.178.x) antwortet oft die <b>lokale</b> FritzBox statt der entfernten (optional, Kostal-Tunnel unabhaengig).');
                }
            } catch (e) {
    ...
    async _connectWg() {
        try {
            ...
            if (existing.ifaceUp) {
                this._log('INFO', 'WG', 'Bestehendes Interface ' + WG_IFACE + ' wird neu geladen \u2026');
            ...
            if (st.connected) {
                ...
                this._log('WARN', 'WG', 'Interface aktiv, aber noch kein Handshake mit Peer.');
            }
            return st.connected;
        } catch (e) {
