...
'.table-wrap{width:100%;max-width:100%;overflow-x:auto;margin-bottom:20px;}' +
...
'.hint code{background:#0e1628;border-radius:4px;padding:2px 6px;color:var(--accent);font-family:monospace;white-space:nowrap;}';
...
'    tb+="<tr><td title=\'"+esc(t.name)+"\'><strong>"+esc(t.name)+"</strong></td>"' +
'      +"<td class=\'mono\' title=\'127.0.0.1:"+esc(t.localPort)+"\'>:"+esc(t.localPort)+"</td>"' +
'      +"<td class=\'mono\' title=\'"+esc(t.remoteHost)+":"+esc(t.remotePort)+"\'>"+esc(t.remoteHost)+":"+esc(t.remotePort)+"</td>"' +
'      +"<td>"+t.active+"/"+t.total+"</td>"' +
'      +"<td class=\'mono\'>"+fmt(t.rxBytes)+" / "+fmt(t.txBytes)+"</td>"' +
...
'<div class="table-wrap"><table><thead><tr><th>Name</th><th>IP</th><th>MAC</th><th>Interface</th><th>Status</th></tr></thead>' +
