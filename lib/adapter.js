...
function sanitizeWgConfig(raw, opts = {}) {
    ...
    const fritzHost   = opts.fritzHost || '';
    if (tunnelHosts.length && /^AllowedIPs\s*=/mi.test(cfg)) {
        const hosts = [...new Set([...tunnelHosts, fritzHost].filter(Boolean))];
        const narrow  = hosts.map(h => h + '/32').join(', ');
        if (/AllowedIPs\s*=\s*192\.168\.178\.0\/24/i.test(cfg)) {
            cfg = cfg.replace(/^AllowedIPs\s*=.+\n?/gim, 'AllowedIPs = ' + narrow + '\n');
            warnings.push('AllowedIPs von 192.168.178.0/24 auf Hosts eingeschraenkt: ' + hosts.join(', '));
        }
    }
    return { cfg, warnings };
}
...
class FritzWireguardLogic {
    ...
    _sanitizeOpts(cfg) {
        ...
        return {
            ...
            fritzHost:   cfg.fritzHost || this.config.fritzHost || ''
        };
    }
