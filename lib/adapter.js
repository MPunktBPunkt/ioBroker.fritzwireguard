...
/** Which kernel device would be used for host (e.g. eth0 vs fritzwg). */
function routeDev(host) {
    return execWg('ip route get ' + host).then(out => {
        const m = String(out).match(/\bdev\s+(\S+)/);
        return m ? m[1] : null;
    }).catch(() => null);
}
    ...
    start(t) {
        ...
        const server = net.createServer(local => {
            ...
            const remote = net.connect(parseInt(t.remotePort), t.remoteHost, () => {
                // Per-connection traffic is routine (e.g. Kostal polls every 30s) —
                // keep out of the ioBroker system log; still in the adapter log buffer.
                this._log('DEBUG', 'TUNNEL',
...
class FritzWireguardLogic {
    ...
    async _pollFritzBox() {
        ...
        // Same local/remote subnet: AllowedIPs are narrowed to tunnel hosts only,
        // so fritzHost (.1) stays on eth0 → TR-064 would hit the *local* box.
        const via = await routeDev(h);
        if (via && via !== WG_IFACE) {
            this._cache.fritzbox = {};
            this._cache.devices = [];
            this._cache.tr064 = {
                ok: false,
                reason: 'FritzBox ' + h + ' wird \u00fcber ' + via + ' erreicht (nicht ' + WG_IFACE +
                    ') \u2014 bei gleichem Subnetz trifft TR-064 die lokale Box. ' +
                    'Ger\u00e4teliste/WAN-Infos sind daher deaktiviert; Tunnel zu einzelnen Hosts funktionieren weiter.'
            };
            const now = Date.now();
            if (!this._tr064SkipLogAt || now - this._tr064SkipLogAt > 10 * 60 * 1000) {
                this._tr064SkipLogAt = now;
                this._log('WARN', 'TR064', this._cache.tr064.reason);
            }
            return;
        }
        ...
        try {
            ...
            if (m) { await set('fritzbox.modelName', m); modelOk = true; }
            if (f) await set('fritzbox.firmwareVersion', f);
        } catch (e) { this._log('WARN', 'TR064', 'DeviceInfo: ' + e.message); }

        this._cache.tr064 = modelOk
            ? { ok: true, reason: null }
            : { ok: false, reason: 'TR-064 DeviceInfo ohne Modell \u2014 Zugangsdaten pr\u00fcfen oder gleiches Subnetz (lokale Box).' };
    ...
    _startServer() {
        ...
        this._server = http.createServer(async (req, res) => {
            ...
            if (n === '/api/status') {
                await this._pollWg();
                const tunnels = this._tunnelMgr
                    ? this._tunnelMgr.statusAll(this.config.tunnels || [])
                    : [];
                const tunEnabled = tunnels.filter(t => t.enabled);
                return this._json(res, {
                    wg: this._cache.wg,
                    fritzbox: this._cache.fritzbox,
                    devices: this._cache.devices,
                    tr064: this._cache.tr064 || { ok: false, reason: null },
                    tunnels: {
                        total: tunEnabled.length,
                        running: tunEnabled.filter(t => t.running).length,
                        targets: tunEnabled.map(t => ({
                            name: t.name || '',
                            host: t.remoteHost || '',
                            port: t.remotePort || 0,
                            running: !!t.running
...
'  if(n==="logs")loadLogs();if(n==="tunnel")loadTunnels();if(n==="daten")loadStatus();if(n==="nodes")loadStatus();if(n==="system")loadStatus();' +
'}' +
...
'  try{await fetch("/api/poll",{method:"POST"});}catch(e){}' +
'  return loadStatus();' +
'}' +
'async function loadStatus(){' +
'  try{var r=await fetch("/api/status");var d=await r.json();' +
'  var wg=d.wg||{};var fb=d.fritzbox||{};var devs=d.devices||[];var tr=d.tr064||{};var tun=d.tunnels||{};' +
...
'  var fbC="";' +
'  if(tr.ok===false&&tr.reason){' +
'    fbC+="<div class=\'hint\' style=\'margin:0 0 12px\'><strong>TR-064 nicht verf\u00fcgbar</strong><br>"+esc(tr.reason)+"</div>";' +
'  }else{' +
...
'    if(!fbC)fbC="<div class=\'hint\' style=\'margin:0\'>Noch keine FritzBox-Daten (TR-064 optional).</div>";' +
'  }' +
...
'  var targets=tun.targets||[];' +
'  if(!tb&&targets.length){' +
'    targets.forEach(function(t){' +
'      tb+="<tr><td data-label=\'Name\'>"+esc(t.name||t.host)+"</td><td data-label=\'IP\'>"+esc(t.host)+"</td>"' +
'        +"<td data-label=\'MAC\' style=\'color:var(--muted)\'>\u2014</td>"' +
'        +"<td data-label=\'Interface\'>Tunnel :"+esc(t.port)+"</td>"' +
'        +"<td data-label=\'Status\'><span class=\'badge "+(t.running?"active":"inactive")+"\'>"+(t.running?"Tunnel aktiv":"Gestoppt")+"</span></td></tr>";' +
'    });' +
'  }' +
'  var nb=document.getElementById("nbody");' +
'  if(nb){' +
'    if(tb)nb.innerHTML=tb;' +
'    else{' +
'      var msg=tr.reason?esc(tr.reason):"Keine Ger\u00e4te (TR-064 Hosts-Liste leer oder nicht erreichbar).";' +
'      nb.innerHTML="<tr><td colspan=\'5\' style=\'text-align:center;color:var(--muted);padding:20px;line-height:1.5\'>"+msg+"</td></tr>";' +
'    }' +
'  }' +
'  var hint=document.getElementById("nodes-hint");' +
'  if(hint){' +
'    if(devs.length)hint.textContent="";' +
'    else if(targets.length)hint.innerHTML="TR-064-Ger\u00e4teliste leer \u2014 zeige konfigurierte Tunnel-Ziele ("+targets.length+"). Die 2 Verbindungen sind die Port-Tunnel, nicht die Hosts-API.";' +
'    else if(tr.reason)hint.textContent="";' +
'    else hint.textContent="";' +
'  }' +
'  var si=document.getElementById("sinfo");' +
'  if(si)si.innerHTML=ir("Adapter","FritzWireguard")+ir("Version","' + v + '")' +
'    +ir("Tunnel aktiv",(tun.running||0)+"/"+(tun.total||0))' +
'    +ir("TR-064 Ger\u00e4te",devs.length)+ir("TR-064 aktiv",devs.filter(function(dv){return dv.active;}).length)' +
'    +(tr.ok===false&&tr.reason?ir("TR-064","nicht verf\u00fcgbar"):ir("TR-064",tr.ok?"ok":"\u2014"));' +
...
'<button class="btn" onclick="refreshStatus()">&#8635; Aktualisieren</button></div>' +
'<p id="nodes-hint" style="font-size:0.82rem;color:var(--muted);margin:0 0 12px;line-height:1.45;"></p>' +
...
function initInstance(adapter) {
    ...
    adapter._cache     = { wg: {}, fritzbox: {}, devices: [], tr064: { ok: false, reason: null } };
